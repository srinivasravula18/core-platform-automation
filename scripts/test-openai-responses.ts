...
async function testSchemaValidHappyPath() {
  console.log(
    "1. Schema-valid happy path — object mapping + cached-token usage split",
  );
  ...
  eq(
    ...
    60,
    "usage.inputTokens excludes cached tokens (100 - 40 = 60)",
  );
  eq(
    result.usage.cacheReadTokens,
    40,
    "usage.cacheReadTokens === cached_tokens (40)",
  );
  eq(
    result.usage.outputTokens,
    20,
    "usage.outputTokens maps from output_tokens",
  );
...
async function testRefusalPath() {
  console.log(
    "2. Refusal path — throwIfRefused throws, throwIfSchemaInvalid is a no-op",
  );
  ...
  ok(
    threw instanceof WorkflowRuntimeError,
    "throwIfRefused throws a WorkflowRuntimeError",
  );
  ok(
    threw instanceof WorkflowRuntimeError &&
      threw.errorClass === WORKFLOW_ERROR_CLASSES.MODEL_REFUSAL,
    "thrown error.errorClass === MODEL_REFUSAL",
  );

  let schemaInvalidThrew = false;
  try {
    throwIfSchemaInvalid(result);
  } catch {
    schemaInvalidThrew = true;
  }
  ok(
    !schemaInvalidThrew,
    "throwIfSchemaInvalid does NOT throw on a refusal result (mutually exclusive)",
  );
}
...
async function testSchemaInvalidPath() {
  console.log(
    "3. Schema-invalid path — throwIfSchemaInvalid throws, throwIfRefused is a no-op",
  );
  ...
  ok(
    threw instanceof WorkflowRuntimeError,
    "throwIfSchemaInvalid throws a WorkflowRuntimeError",
  );
  ok(
    threw instanceof WorkflowRuntimeError &&
      threw.errorClass === WORKFLOW_ERROR_CLASSES.SCHEMA_INVALID_OUTPUT,
  ...
  ok(
    !refusedThrew,
    "throwIfRefused does NOT throw on a schema-invalid (non-refusal) result",
  );
}
...
async function testSdkThrowPath() {
  console.log(
    "4. SDK-throw path — exception caught internally, never propagates",
  );
  ...
  ok(
    !propagated,
    "the SDK exception does not propagate out of callOpenAIResponsesStructured",
  );
  ok(
    result !== null && result.schemaValid === false,
    "the caught-exception result reports schemaValid: false",
  );
  ok(
    result !== null &&
      !!result.rawContent &&
      result.rawContent.includes("boom"),
    "rawContent contains the caught error message",
  );
}
...
async function testRetryOwnershipAndCancellation() {
  console.log(
    "5. Retry ownership — store:false, text.format present, signal wiring, maxRetries:0 construction",
  );
  ...
  ok(
    capturedBody.store === false,
    "body.store === false (explicit no-storage policy)",
  );
  ok(
    capturedBody.text?.format !== undefined,
    "body.text.format is present (zodTextFormat result)",
  );
  ok(
    capturedOptions?.signal === controller.signal,
    "the same AbortSignal instance reaches client.responses.parse as options.signal",
  );
  ...
  const responsesClientPath = path.join(
  ...
  ok(
    /new OpenAI\(\{\s*apiKey:\s*opts\.apiKey,\s*maxRetries:\s*0\s*\}\)/.test(
      source,
    ),
  ...
  const constructedTheSameWay = new OpenAI({
    apiKey: "test-key-not-real",
    maxRetries: 0,
  });
  ok(
    constructedTheSameWay.maxRetries === 0,
    "constructing an OpenAI client the same way resolves maxRetries === 0 (no network call made)",
  );
}
...
async function testRedaction() {
  ...
  const result = await callOpenAIResponsesStructured({
    ...
    system: distinctiveSystem,
    client,
  });

  const serialized = JSON.stringify(result);
  ok(
    !serialized.includes(distinctivePrompt),
    "serialized result does not contain the distinctive prompt text",
  );
  ok(
    !serialized.includes(distinctiveSystem),
    "serialized result does not contain the distinctive system text",
  );
}
...
function testPromptBudgetAllFit() {
  ...
  const result = assemblePromptBudget(candidates, {
    model: "gpt-5.4",
    reservedForOutput: 1000,
    reservedForSystemAndInstructions: 1000,
  });

  eq(result.included.length, 3, "all 3 candidates are included");
  ok(
    result.entries.every((e) => e.included === true),
    "every entry has included: true",
  );
  ok(
    result.entries.every((e) => /\d/.test(e.reason)),
    "every reason mentions a numeric token count",
  );
...
function testPromptBudgetExclusions() {
  console.log(
    '7b. assemblePromptBudget — deterministic exclusions by priority, "excluded" + numeric reason',
  );
  ...
  const result = assemblePromptBudget(candidates, {
    model,
    reservedForOutput,
    reservedForSystemAndInstructions,
  });

  eq(
    result.included.map((c) => c.key),
    ["high"],
    "only the highest-priority candidate survives (60 <= 100, next would exceed)",
  );
  const excludedEntries = result.entries.filter((e) => !e.included);
  eq(
    excludedEntries.map((e) => e.key),
    ["medium", "low"],
    "medium and low priority candidates are excluded",
  );
  for (const entry of excludedEntries) {
    ok(
      entry.reason.includes("excluded"),
      `excluded entry "${entry.key}" reason contains the word "excluded"`,
    );
    ok(
      /\d/.test(entry.reason),
      `excluded entry "${entry.key}" reason contains a numeric token figure`,
    );
  }
}
...
function testPromptBudgetStableOrdering() {
  console.log(
    "7c. assemblePromptBudget — same-priority candidates preserve original input order",
  );
  ...
  eq(
    result.included.map((c) => c.key),
    ["first", "second", "third"],
    "stable sort: equal-priority candidates keep input order",
  );
  eq(
    result.entries.map((e) => e.key),
    ["first", "second", "third"],
    "entries preserve the same stable order",
  );
}
...
function testPromptBudgetVersion() {
  ...
  ok(
    Number.isInteger(PROMPT_BUDGET_VERSION) && PROMPT_BUDGET_VERSION > 0,
    "PROMPT_BUDGET_VERSION is a positive integer",
  );
}

async function testProviderToolContinuity() {
  console.log(
    "8. OpenAI provider Responses tool continuity - native items, store:false, encrypted reasoning",
  );
  const bodies: any[] = [];
  const reasoning = {
    id: "rs_1",
  ...
  const responses = [
    ...
    },
    {
      ...
      model: "gpt-5.4",
      status: "completed",
      output: fakeMessageOutput([{ type: "output_text", text: "done" }]),
      output_text: "done",
      incomplete_details: null,
      usage: fakeUsage(),
    },
  ...
  eq(
    ...
    "function_call maps by call_id",
  );
  ok(
    first.providerItems?.[0] === reasoning,
    "native reasoning output is retained for continuation",
  );
  ...
  const second = await provider.chatWithTools({
    messages: [
      { role: "user", content: "look it up" },
      {
        ...
        providerItems: first.providerItems,
      },
  ...
  ok(
    bodies.every((body) => body.store === false),
    "every provider Responses request sets store:false",
  );
  ok(
    bodies.every((body) =>
      body.include?.includes("reasoning.encrypted_content"),
    ),
    "stateless requests include encrypted reasoning",
  );
  ok(
    bodies[0].tools?.[0]?.name === "lookup" && !bodies[0].tools?.[0]?.function,
    "Responses function tool uses flat shape",
  );
  ok(
    bodies[1].input[1] === reasoning,
    "next request replays the native reasoning item verbatim",
  );
  ok(
    bodies[1].input.some(
      (item: any) =>
    ...
    "tool output correlates with function call_id",
  );
}
...
async function testProviderStructuredOutput() {
  console.log(
    "9. OpenAI provider structured output - native schema enforcement",
  );
  let capturedBody: any = null;
  const provider = new OpenAIProvider("test-key-not-real", "gpt-5.4");
  (provider as any).client = {
  ...
  await provider.generateObject({
    prompt: "Improve this case end to end.",
    schema: TEST_SCHEMA,
  });

  ok(
    /json/i.test(String(capturedBody.input)),
    'json_object request includes "JSON" in Responses input',
  );
  ok(
    capturedBody.text?.format?.type === "json_schema",
    "request uses strict json_schema response format",
  );
  ok(
    capturedBody.text?.format?.strict === true,
    "structured output schema is strict",
  );
  ok(
    capturedBody.text?.format?.schema?.required?.includes("title"),
    "structured output requires the schema fields",
  );
}
...
async function testProviderDynamicRecordFallback() {
  console.log(
    "10. OpenAI provider dynamic records - JSON mode fallback with local validation",
  );
  let capturedBody: any = null;
  const provider = new OpenAIProvider("test-key-not-real", "gpt-5.4");
  (provider as any).client = {
  ...
  ok(
    capturedBody.text?.format?.type === "json_object",
    "propertyNames schema falls back to JSON mode",
  );
  eq(
    result.object,
    { params: { tag: "@ui" } },
    "JSON-mode response is still validated by Zod",
  );
}
