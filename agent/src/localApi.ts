...
export function startLocalApi(deps: { log: Logger; loggerHandle: LoggerHandle; config: AgentConfig; conn: ConnectionManager }): void {
  ...
  let openBrowser: ChildProcess | null = null;
  ...
  app.post('/record/stop', (req, res) => {
    ...
    if (recordingId) conn.recorder.stop(recordingId);
    else conn.recorder.stopAll();
    res.json({ ok: true });
  });
  ...
  app.post('/cancel', (req, res) => {
    ...
    if (jobId) conn.runner.cancel(jobId);
    res.json({ ok: true });
  });
  ...
  app.post('/pauses/:id/resolve', (req, res) => {
    ...
    if (!jobId || !pauseId) return res.status(400).json({ error: 'jobId and pause id are required.' });
  ...
  app.post('/browser/close', (_req, res) => {
    if (openBrowser?.pid) {
      if (process.platform === 'win32') spawn('taskkill.exe', ['/pid', String(openBrowser.pid), '/t', '/f'], { stdio: 'ignore' });
      else openBrowser.kill('SIGTERM');
      openBrowser = null;
    }
    res.json({ ok: true });
  });
