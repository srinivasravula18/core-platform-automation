...
export function measuredWindowFile(workDir: string): string {
  return path.join(workDir, 'codegen-window.json');
}

function measuredWindow(workDir: string): string {
  try {
...
interface ActiveRecording {
  ...
  lastScript: string;
  paused: boolean;
  /** Lines as they stood when the current pause began, to diff whatever arrives while paused. */
  pausedLines: string[] | null;
  /** Line windows the recorder wrote during each pause, dropped from every later read. */
  dropped: Array<{ start: number; lines: string[] }>;
}

/** The lines the recorder inserted into `before` to produce `after`, as one contiguous window. */
function insertedWindow(before: string[], after: string[]): { start: number; lines: string[] } | null {
 ...
 */
function visibleScript(state: ActiveRecording, raw: string): string {
  ...
  for (const window of windows) {
    ...
    if (found.join(NEWLINE) === window.lines.join(NEWLINE)) lines.splice(window.start, window.lines.length);
  }
...
export class Recorder {
  ...
  start(recordingId: string, url: string, browser = 'chromium', browserPermissions?: BrowserPermissionSettings, videoJobId = ''): void {
    ...
    const state: ActiveRecording = { child, outputPath, profileDir: codegenProfileDir(this.workDir, recordingId), videoDir, videoJobId, lastScript: '', paused: false, pausedLines: null, dropped: [], poll: setInterval(() => this.tick(recordingId), 1000) };
  ...
  private readScript(state: ActiveRecording): string {
    try { return fs.readFileSync(state.outputPath, 'utf-8'); } catch { return ''; }
  }
  ...
  private command(state: ActiveRecording, command: 'stop'): boolean {
  ...
  resume(recordingId: string): void {
    ...
    if (window) state.dropped.push(window);
