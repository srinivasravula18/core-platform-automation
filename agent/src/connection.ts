  ...
  private connect(): void {
    ...
    ws.on('unexpected-response', (_req, res) => {
      ...
      void refresh.finally(() => {
        if (this.heartbeat) clearInterval(this.heartbeat);
        if (!this.closing && this.ws === ws) this.scheduleReconnect();
      });
    });
  }
  ...
  private onMessage(raw: import('ws').RawData): void {
    ...
    switch (frame.type) {
      ...
      case 'pause.resume':
        ...
        break;
      case 'pause.cancel':
        if (frame.payload?.jobId && frame.payload?.pauseId) this.runner.cancelPause(frame.payload.jobId, frame.payload.pauseId, Number(frame.payload.attempt) || 1, String(frame.payload.resolvedBy || 'cloud'));
        break;
      case 'cancel':
