...
function classify(file: string): ArtifactKind {
  ...
  if (lower.endsWith('.json') || lower.endsWith('.log') || lower.endsWith('.txt')) return 'log';
  return 'other';
}
...
export function collectArtifacts(runDir: string): CollectedArtifact[] {
  const files: string[] = [];
  ...
  if (fs.existsSync(htmlIndex)) files.push(htmlIndex);
