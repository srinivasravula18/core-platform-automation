...
function ffmpegReady() {
    try {
        return fs.readdirSync(browsersRoot()).some((d) => d.startsWith('ffmpeg-'));
    }
    catch {
        return false;
    }
 ...
 */
export function prewarmBrowser(log) {
    const channel = chromiumChannel() || 'chromium';
    if (!bundledChromiumReady() && !systemChromePath())
        return;
...
export function ensureBrowsers(log, cwd) {
    ...
    child.once('close', (code) => {
        if (code === 0)
            log.info({ installed: missing }, 'browser components installed — record & run ready');
        else
            log.error({ code }, `install failed — run \`npx playwright install ${missing.join(' ')}\` in the agent folder`);
    });
