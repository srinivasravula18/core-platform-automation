...
const resolveAppId = (token) => {
  ...
  recordSlowRequest(res);
  const ok = check(res, {
    "apps list status is 200": (r) => r.status === 200
  });
  if (!ok) return null;
  const body = res.json();
...
const resolveObjectApi = (token, appId) => {
  ...
  recordSlowRequest(res);
  const ok = check(res, {
    "objects list status is 200": (r) => r.status === 200
  });
  if (!ok) return null;
  const body = res.json();
...
const resolveListViewId = (token, appId, objectApi) => {
  ...
  recordSlowRequest(res);
  const ok = check(res, {
    "list views status is 200": (r) => r.status === 200
  });
  if (!ok) return null;
  const body = res.json();
  return body?.items?.[0]?.id ?? null;
...
const exportPdf = (token, appId, objectApi, listViewId) => {
  const payload = listViewId
    ? { format: "pdf", list_view_id: listViewId }
    : { format: "pdf", columns: ["id", "name", "status", "created_at"] };

  let res = http.post(
    `${API_BASE}/api/apps/${appId}/objects/${objectApi}/list-views/export`,
...
export default function (data) {
  ...
  const res = exportPdf(token, appId, objectApi, listViewId);

  const contentType = res.headers["Content-Type"] || res.headers["content-type"] || "";
  const contentDisposition =
    res.headers["Content-Disposition"] || res.headers["content-disposition"] || "";
  const bodyHead = typeof res.body === "string" ? res.body.slice(0, 8) : "";

  const ok = check(res, {
    "export status is 200": (r) => r.status === 200,
    "export is pdf": () =>
      String(contentType).includes("application/pdf") || bodyHead.startsWith("%PDF-"),
    "export is attachment (optional)": () =>
      !contentDisposition ||
      (String(contentDisposition).includes("attachment") &&
        String(contentDisposition).includes(".pdf"))
  });

  if (!ok) {
    const body = typeof res.body === "string" ? res.body.slice(0, 200) : "";
    console.error(
      `pdf export failed: status=${res.status} appId=${appId} object=${objectApi} listViewId=${
        listViewId ?? "none"
      } contentType=${contentType} contentDisposition=${contentDisposition} bodyHead=${body}`
    );
  }
}
...
export function handleSummary(data) {
  ...
  const html = buildHtmlReport({
    title: "List View Export PDF Test Report",
    summaryLines,
    latencyLines,
    slowLines
  });
  return {
    stdout: `${lines.join("\n")}\n`,
    "list-view-export-pdf-test-summary.html": html
  };
}
