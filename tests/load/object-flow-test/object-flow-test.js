...
const ADMIN_APP_ID = __ENV.ADMIN_APP_ID || "";
...
const resolveAppId = (token) => {
  if (ADMIN_APP_ID) return ADMIN_APP_ID;
...
const buildObjectPayload = () => {
  const stamp = `${__VU}_${__ITER}_${Date.now()}_${Math.floor(Math.random() * 1e6)}`;
  return {
    api_name: `load_obj_${stamp}`,
    label: `Load Obj ${stamp}`,
    id_prefix: buildIdPrefix(),
    global_search_enabled: false,
    inline_edit_enabled: true,
    list_view_relationship_depth: 5
  };
};
...
export default function () {
  ...
  const ok = check(res, {
    "object create status is 201": (r) => r.status === 201,
    "object create returns id": (r) => {
      try {
        const body = r.json();
  ...
  if (!ok) {
    ...
    console.error(
      `object create failed: status=${status} api_name=${payload.api_name} id_prefix=${payload.id_prefix} body=${body}`
    );
  }
}
...
export function handleSummary(data) {
  ...
  return {
    stdout: `${lines.join("\n")}\n`,
    "object-flow-test-summary.html": html
  };
}
