...
export function toReportHTML(report: any, context: ReportContext = {}): string {
  ...
  const templateRows = features.map((feature) => {
    ...
    return `<tr><td>${esc(feature)}</td><td>—</td><td>—</td><td>${esc(resultFor(steps))}</td><td>${esc(duration(steps.reduce((sum: number, step: any) => sum + Number(step.durationMs || 0), 0)))}</td><td>${esc(tags.join(' ') || '—')}</td></tr>`;
  }).join('');
  ...
  <h2 class="section">Templates executed this run</h2><table><thead><tr><th>Template</th><th>ATS score</th><th>AI / Human</th><th>Result</th><th>Time</th><th>Tag</th></tr></thead><tbody>${templateRows || '<tr><td colspan="6">No templates recorded.</td></tr>'}</tbody></table>
