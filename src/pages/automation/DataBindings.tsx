...
import { Database, Link2, Play, Redo2, Search, Trash2, Undo2, Upload, X } from 'lucide-react';
...
export default function DataBindings() {
  ...
  useEffect(() => {
    setSelectedRows([]);
    setIsPreviewOpen(Boolean(datasetId));
  ...
  return <div className="flex min-h-0 flex-1 flex-col gap-4">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><h1 className="text-xl font-semibold text-[var(--text-primary)]">Automation Data</h1><p className="mt-1 text-sm text-[var(--text-muted)]">Edit recorded inputs directly, or optionally bind them to dataset columns.</p></div>
    ...
    {!recordingId ? <div className="rounded-lg border border-dashed border-[var(--border)] p-10 text-center text-sm text-[var(--text-muted)]"><Database className="mx-auto mb-2 h-6 w-6" />Select a recorded script to edit its captured input values.</div> :
      <div className={`grid min-h-0 gap-4 ${dataset ? 'xl:grid-cols-[1fr_20rem]' : ''}`}>
        <div className="rounded-lg border border-[var(--border)] bg-[var(--bg-card)]">
          ...
          {!steps.length && <div className="p-8 text-center text-sm text-[var(--text-muted)]">This script has no fill, select, check, upload, or keyboard input actions to edit.</div>}
          {steps.map((step) => {
            ...
            return <div key={step.id} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); void mapColumn(step.id, event.dataTransfer.getData('column')); }} className="flex flex-wrap items-center gap-3 border-b border-[var(--border)] p-3 last:border-0 focus-within:ring-1 focus-within:ring-[var(--accent)]">
              ...
              <div className="min-w-40 flex-1"><div className="text-sm text-[var(--text-primary)]">{step.metadata.label || step.locator}</div><div className="text-xs text-[var(--text-muted)]">{mapping ? `{{${column?.name || 'Missing column'}}}` : dataset ? 'Drop or choose a column' : step.readOnly ? 'Read-only recorded action' : 'Edit the captured value'}</div></div>
              ...
              <button type="button" disabled={!step.canUndo} title="Undo last value change" onClick={() => void history(step.id)} className="inline-flex items-center gap-1 rounded border border-[var(--border)] px-2 py-1 text-xs disabled:cursor-not-allowed disabled:opacity-40"><Undo2 className="h-3.5 w-3.5" />Undo</button>
              <button type="button" disabled={!step.canRedo} title="Redo last undone change" onClick={() => void history(step.id, true)} className="inline-flex items-center gap-1 rounded border border-[var(--border)] px-2 py-1 text-xs disabled:cursor-not-allowed disabled:opacity-40"><Redo2 className="h-3.5 w-3.5" />Redo</button>
            </div>;
          })}
        </div>
        {dataset && <aside className="rounded-lg border border-[var(--border)] bg-[var(--bg-card)] p-3">
    ...
    {dataset && !isPreviewOpen && (
      <button type="button" onClick={() => setIsPreviewOpen(true)} className="self-start rounded-md border border-[var(--border)] bg-[var(--bg-card)] px-3 py-2 text-sm hover:border-[var(--accent)]">
        Show data preview
      </button>
    )}
    {dataset && isPreviewOpen && <section aria-label="Dataset preview" className="min-h-0 overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--bg-card)]">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] p-3">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          <button type="button" onClick={() => setIsPreviewOpen(false)} aria-label="Close data preview" title="Close data preview" className="rounded border border-[var(--border)] p-2 text-[var(--text-muted)] hover:bg-[var(--bg-secondary)] hover:text-[var(--text-primary)]">
            <X className="h-4 w-4" />
          </button>
        </div>
      </div>
      {batch && <div className="border-b border-[var(--border)] px-3 py-2 text-xs text-[var(--text-secondary)]">Batch {batch.status}: {batch.summary?.passed || 0} passed · {batch.summary?.failed || 0} failed · {batch.summary?.running || 0} running · {batch.summary?.queued || 0} queued</div>}
      <div className="max-h-[min(50vh,32rem)] overflow-auto">
        <table className="min-w-max text-xs">
          <thead className="sticky top-0 z-10 bg-[var(--bg-card)]"><tr><th className="whitespace-nowrap p-2 text-left">Run</th><th className="whitespace-nowrap p-2 text-left">Row</th>{dataset.columns.map((column: any) => <th key={column.id} className={`whitespace-nowrap p-2 text-left ${column.name === 'Step Detail' ? 'min-w-[28rem]' : 'min-w-36'}`}>{column.name}</th>)}</tr></thead>
          <tbody>{rows.map((row) => <tr key={row.id} className="border-t border-[var(--border)] align-top"><td className="p-2"><input aria-label={`Select row ${row.rowNumber}`} type="checkbox" checked={selectedRows.includes(row.rowNumber)} onChange={() => setSelectedRows((current) => current.includes(row.rowNumber) ? current.filter((number) => number !== row.rowNumber) : [...current, row.rowNumber])} /></td><td className="p-2">{row.rowNumber}</td>{dataset.columns.map((column: any) => <td key={column.id} className={`p-2 ${column.name === 'Step Detail' ? 'max-w-[36rem] whitespace-pre-wrap break-words leading-5' : 'max-w-72 whitespace-nowrap overflow-hidden text-ellipsis'}`}>{row.values[column.id] || '—'}</td>)}</tr>)}</tbody>
        </table>
      </div>
    </section>}
  </div>;
