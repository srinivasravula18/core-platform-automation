...
function activitySteps(events: ActivityEvent[]): ActivityStep[] {
  ...
  for (const event of events) {
    ...
    if (payload.kind === 'tool_started') {
      const index = steps.push({ key: String(event.seq), kind: payload.kind, label: friendlyTool(name), status: 'running', tool: payload.tool }) - 1;
      pending.set(name, [...(pending.get(name) || []), index]);
      continue;
    }
    if (payload.kind === 'tool_completed') {
      const indexes = pending.get(name) || [];
      const index = indexes.shift();
      if (index !== undefined) {
        steps[index] = { ...steps[index], status: payload.tool?.error ? 'failed' : 'completed', tool: payload.tool };
        pending.set(name, indexes);
      } else {
        steps.push({ key: String(event.seq), kind: payload.kind, label: friendlyTool(name), status: payload.tool?.error ? 'failed' : 'completed', tool: payload.tool });
      }
      continue;
    }
...
export const AgentActivity = memo(function AgentActivity({
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        const response = await fetch(`/api/controller/activity/for-conversation/${encodeURIComponent(conversationId)}`, { cache: 'no-store' });
        if (!response.ok) return;
        const data = await response.json();
        if (disposed) return;
        const matching = (Array.isArray(data.events) ? data.events : []).filter((event: ActivityEvent) =>
