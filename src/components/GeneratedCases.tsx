...
function readinessClasses(status?: string): string {
...
function stepProofClasses(status?: string): string {
  switch ((status || '').toLowerCase()) {
    case 'verified':
      return 'border-emerald-500/30 text-emerald-400 bg-emerald-500/10';
    case 'metadata-backed':
          ...
          {editing !== i && (c.automationReadiness || c.proofSummary) && (
            <div className="mt-2 flex flex-wrap items-center gap-2">
              {c.automationReadiness && (
                <span className={`rounded-md border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${readinessClasses(c.automationReadiness)}`}>
                  {c.automationReadiness}
                </span>
              )}
                            ...
                            <div className="space-y-1">
                              ...
                              {s.proofStatus && (
                                <div className="flex flex-wrap items-center gap-1.5">
                                  <span className={`rounded-md border px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wider ${stepProofClasses(s.proofStatus)}`}>
                                    {s.proofStatus}
                                  </span>
