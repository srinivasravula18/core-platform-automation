  ...
  return <div className="qa-report">
    ...
    <div className="qa-report-layout">
      ...
      <main className="qa-report-main">
        ...
        <p className="qa-note">Template-level execution, scoring, origin, result, time, and tags for this run.</p>
        <div className="qa-table-scroll"><table className="qa-template-table"><thead><tr><th>Template</th><th>ATS score</th><th>AI / Human</th><th>Result</th><th>Time</th><th>Tag</th></tr></thead><tbody>{templates.map((template) => <tr key={template.name}><td><b>{template.name}</b></td><td><span className="qa-ats-pill">—</span><div className="qa-ats-breakdown">—</div></td><td><span className="qa-ats-pill">—</span></td><td><span className={`qa-result-text ${template.outcome}`}>{template.outcome === 'passed' ? 'Pass' : template.outcome === 'failed' ? 'Fail' : 'Skipped'}</span></td><td>{duration(template.durationMs)}</td><td className="qa-template-tags">{template.tags.map((tag) => tag.startsWith('@') ? tag : `@${tag}`).join(' ') || '—'}</td></tr>)}</tbody></table></div>
