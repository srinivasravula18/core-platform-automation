  ...
  return (
    <div className="relative inline-flex">
      <button
        ...
        aria-haspopup="menu"
        aria-expanded={open}
        className="p-1 rounded hover:bg-[var(--border)] text-[var(--text-muted)] transition-colors focus-visible:ring-2 focus-visible:ring-sky-500 focus:outline-none"
      >
          ...
          >
            {items.map((item) => (
              <button
                ...
                className={`block w-full px-3 py-2 text-left text-sm transition-colors hover:bg-[var(--bg-secondary)] focus-visible:bg-[var(--bg-secondary)] focus:outline-none ${
                  item.danger ? 'text-red-400 hover:bg-red-500/10 focus-visible:bg-red-500/10' : 'text-[var(--text-primary)]'
                }`}
              >
                {item.label}
              </button>
