  ...
  return (
    <div className="relative inline-flex">
      <button
        ...
        aria-haspopup="true"
        aria-expanded={open}
        className="p-1 rounded hover:bg-[var(--border)] text-[var(--text-muted)] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-1"
      >
          ...
          >
            {items.map((item) => (
              <button
                ...
                className={`block w-full px-3 py-2 text-left text-sm hover:bg-[var(--bg-secondary)] focus-visible:outline-none focus-visible:bg-[var(--bg-secondary)] ${item.danger ? "text-red-400 hover:bg-red-500/10" : "text-[var(--text-primary)]"}`}
              >
                {item.label}
              </button>
            ))}
