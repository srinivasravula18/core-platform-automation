...
export function MetricTiles({
  ...
  return (
    <div className={cn('mb-4 grid grid-cols-2 gap-3', className)}>
      {items.map((item) => (
        <div key={item.label} className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-4 shadow-sm">
          <span className="flex items-center gap-1.5 text-xs font-medium text-[var(--text-muted)]">
            {item.color && <StatusDot color={item.color} />}
            {item.label}
          </span>
          <div className="mt-1 text-2xl font-bold text-[var(--text-primary)]">{item.value}</div>
        </div>
      ))}
    </div>
  );
