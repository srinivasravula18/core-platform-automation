  ...
  try {
    if (repoPath) {
      ...
      selectorCatalogBlock = rendered
        ? `\n\nREPO UI SELECTOR CATALOG FOR THIS REQUIREMENT - exact strings extracted from source. Candidate scenario steps must use these labels/aria-labels/ids/classes when they refer to UI controls. If a needed control is not listed here, say the control could not be confirmed instead of inventing a label. This catalog grounds candidateScenarios ONLY: never quote an id/class/selector, or comment on which hooks exist, in srsModules or businessRules — those are read by non-engineers:\n${rendered}`
...
- srsModules: organize every business rule into distinct functional modules for the Agent Console's Notion-style Markdown Requirements response. Each module needs a concise title and ordered requirements. Each requirement needs:
  ...
  - sources: the specific file path(s) from the evidence that justify THIS requirement (add a line range like "path:120-140" only when the excerpt makes the lines certain — never guess line numbers). This is the fine-grained code↔requirement trace; keep it to the 1-3 files that actually prove the rule. It is machine trace only and is never displayed, so nothing the reader needs may live here alone.
  - details: optional lines for conditions, defaults, enumerated values, or validation rules. Business facts only — never a file path, id/class/selector, or a note about which test hooks exist; a detail the reader cannot act on is dropped.
