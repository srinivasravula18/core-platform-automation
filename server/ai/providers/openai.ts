...
function openAIJsonFormat(schema: z.ZodTypeAny | undefined, mode: 'text' | 'chat'): any {
  if (!schema) return { type: 'json_object' };
  try {
    const format = mode === 'text'
      ? zodTextFormat(schema, 'structured_output')
    ...
    try {
      if (this.useResponsesApi()) {
        ...
        const response = await this.client.responses.create({
          ...
          ...(jsonMode ? { text: { format: openAIJsonFormat(schema, 'text') } } : {}),
        }, { signal: opts.signal });
      ...
      const completion = await this.client.chat.completions.create(
        {
          ...
          ...(jsonMode ? { response_format: openAIJsonFormat(schema, 'chat') } : {}),
        },
        { signal: opts.signal },
      );
