...
function openAIJsonFormat<T>(format: T): T | { type: "json_object" } {
  return JSON.stringify(format).includes('"propertyNames"')
    ? { type: "json_object" }
    : format;
}

function safeOpenAITextFormat(schema: z.ZodTypeAny): any {
  try {
    return openAIJsonFormat(zodTextFormat(schema, "structured_output"));
  } catch {
    return { type: "json_object" as const };
  }
}

function safeOpenAIResponseFormat(schema: z.ZodTypeAny): any {
  try {
    return openAIJsonFormat(zodResponseFormat(schema, "structured_output"));
  } catch {
    return { type: "json_object" as const };
  }
}
...
function toResponseInput(
  ...
  for (const message of messages) {
    ...
    if (message.role === "assistant" && message.providerItems?.length) {
      input.push(
        ...(message.providerItems as OpenAI.Responses.ResponseInputItem[]),
      );
      continue;
    }
...
export class OpenAIProvider implements AIProvider {
  ...
  constructor(
    apiKey: string,
    defaultModel?: string,
    options?: { baseURL?: string; name?: ProviderName },
  ) {
  ...
  async health(): Promise<ProviderHealth> {
    ...
    if (!this.apiKey) {
      return {
        ...
        error: `${this.name} API key not set`,
        checkedAt,
      };
    }
    try {
      ...
      return {
        ...
        provider: this.name,
        model: this.defaultModel,
        error: this.toProviderError(err).message,
        checkedAt,
      };
    }
  }
  ...
  private async callChat(
    opts: GenerateTextOptions & { images?: ProviderImage[] },
    jsonMode: boolean,
    schema?: z.ZodTypeAny,
  ) {
          ...
          ...(jsonMode
            ? {
                response_format: schema
                  ? safeOpenAIResponseFormat(schema)
                  : { type: "json_object" as const },
              }
            : {}),
        },
  ...
  private async chatWithResponses(
    ...
    const start = Date.now();
    const modelId = this.modelId(opts);
    const tools: OpenAI.Responses.FunctionTool[] | undefined = opts.tools
      ?.length
      ? opts.tools.map((tool) => ({
          type: "function",
          name: tool.name,
  ...
  private toUsageObj(modelId: string, usage?: any) {
    ...
    const totalTokens =
      usage.total_tokens ?? inputTokens + outputTokens + cacheReadTokens;
    const usageObj = {
      inputTokens,
      outputTokens,
      cacheReadTokens,
  ...
  async generateObject<T>(
    ...
    const { content, usage, modelId, finishReason } = await this.callChat(
      { ...opts, system: jsonHint },
      true,
      schemaZ,
    );
    const outputTokens =
      (usage as any)?.completion_tokens ?? (usage as any)?.output_tokens;
    // Length-truncated JSON must FAIL (classified, retried by the orchestrator) — never be salvaged short.
    if (finishReason === "length")
      throw structuredTruncationError(this.name, modelId, outputTokens);
    let parsed: unknown;
    try {
      ...
      if (unterminated)
        throw structuredTruncationError(this.name, modelId, outputTokens);
      if (!json)
        throw classifyError(this.name, 200, "Model did not return valid JSON");
    ...
    parsed = normalizeScriptPayload(
      normalizeTestCasePayload(coerceToSchemaShape(parsed, schemaZ)),
    );
    ...
    try {
      ...
      try {
        validated = schemaZ.parse(
          repairValidationError(parsed, validationError),
        );
        ...
        const issues: any[] = Array.isArray(stillInvalid?.issues)
          ? stillInvalid.issues
          : [];
        const fields = issues
          .slice(0, 4)
        ...
        throw classifyError(
          this.name,
          200,
          `Model response did not match the expected schema${fields ? ` (fields: ${fields})` : ""}.`,
        );
      }
    }
    return {
