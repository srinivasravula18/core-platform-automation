...
- **No feature flags.** Never add env-var switches, dark-launch gates, or `shadow/canary/native` modes. Build the feature so it works and ship it ON. Flags here produced duplicate live paths, a 432-line dead pipeline, and features left silently off for months — all removed on 2026-08-13. If something feels too risky to enable, say so and ask; do not gate it. Never write a test asserting flag-OFF legacy behavior. (`REMOTE_AGENT_V1` / `SESSION_RUN_PROJECTION_V1` are deliberately retained.)
